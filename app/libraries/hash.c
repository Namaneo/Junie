...
{
	MTY_Hash *ctx = calloc(1, sizeof(MTY_Hash));
	ctx->num_buckets = numBuckets == 0 ? HASH_DEFAULT_BUCKETS : numBuckets;

	ctx->buckets = calloc(ctx->num_buckets, sizeof(struct hash_bucket));
...
void *MTY_HashSet(MTY_Hash *ctx, const char *key, void *value)
{
	...
	if (!n) {
		b->nodes = realloc(b->nodes, ++b->num_nodes * sizeof(struct hash_node));
		n = &b->nodes[b->num_nodes - 1];
	}

	n->key = SDL_strdup(key);
	n->val = value;
